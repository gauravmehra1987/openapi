<?php
class ControllerPaymentMagPPPro extends Controller {
...
$this->log->write($order_info);
		//getting payment address information
		$address_id = $this->request->post['address_id'];
		$this->load->model('account/address');
		$payment_address = $this->model_account_address->getAddress($address_id);
		
		//updating order address info
		$order_info['payment_firstname'] = $payment_address['firstname']; // Guntis
		$order_info['payment_lastname'] = $payment_address['lastname']; // Endzelis
		$order_info['payment_company'] = $payment_address['company']; // Guntis Endzelis
		$order_info['payment_address_1'] = $payment_address['address_1']; // Strazdu iela 5
		$order_info['payment_address_2'] = $payment_address['address_2']; // 
		$order_info['payment_postcode'] = $payment_address['postcode']; // 85001
		$order_info['payment_city'] = $payment_address['city']; // Jelgava
		$order_info['payment_zone_id'] = $payment_address['zone_id']; // 4051
		$order_info['payment_zone'] = $payment_address['zone']; // Auce, Auces novads
		$order_info['payment_zone_code'] = $payment_address['zone_code']; // 0460805
		$order_info['payment_country_id'] = $payment_address['country_id']; // 117
		$order_info['payment_country'] = $payment_address['country']; // Latvia
		$order_info['payment_address_format'] = $payment_address['address_format']; // 

		$this->load->model('checkout/mag_order');
		//saving payment address
		$this->model_checkout_mag_order->updateOrderPaymentAddress($this->session->data['order_id'], $order_info);
...
//Uncomment these lines when testing completed
...
//end uncomment lines
		...
		//remove these lines when testing completed
		$message = 'testing PP without real transaction';
		$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('pp_pro_order_status_id'), $message, false);
		$json['success'] = $this->url->link('checkout/success');
		//end remove
